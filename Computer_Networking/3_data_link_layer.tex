...
\section{数据链路层}
数据链路层把\textemph{数据帧(frame)}，从一个节点通过链路（直连网络或\textemph{物理网络}）传给相邻另一个节点（主机或路由器）。
...
\end{itemize}
\item 校验和(checksum)：将所有数据加起来，每16位1组，最高位进位则\textemph{末尾加1}，最后结果\textemph{取反}\\
...
\end{figure}
链路层常用\textemph{CRC}，因为检错率很高，且容易实现（触发器+异或门）
\end{enumerate}
...
\begin{itemize}
	...
	\item 捎带确认(piggybacking)：通信双方\textemph{全双工}方式工作，接收方在发数据给对方时顺便把确认号也告诉对方（两个滑动窗口，两边都要发数据），需要结合下面一起使用
...
\begin{enumerate}
	\item 发送数据帧之前先监听信道。如果信道空闲，\textemph{立即}发送。
...
工作流程如下
\begin{itemize}
\item 先确定根网桥，即\textemph{BID(Bridge ID)最小}的
...
\item 网桥只在根端口和指定端口之间转发\textemph{数据}帧，不可通过阻塞端口
...
每次扩散扩散到帧内指定的端口或\textemph{干道端口}，同样查MAC地址表转发。
...
\subsubsection{令牌环网}
令牌环网(token ring)通过在站点之间传递令牌防止冲突并且具有\textemph{优先权}的星形\textemph{LAN}，其标准为IEEE 802.5，take turns protocol，现在有千兆令牌环网。
...
交换机是一个把多个网段连接起来的设备，也称为\textemph{多端口网桥}。
...
\item 无碎片(fragment free)：都没冲突才开始转发，\textemph{最小帧保证}\\
...
\begin{itemize}
	...
	\item 交换机(switch)：\textemph{数据链路层/二层}，MAC地址，建立连接，独享信道，全双工，增加冲突域数量，减少冲突范围大小
