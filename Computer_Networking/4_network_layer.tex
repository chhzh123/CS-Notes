...
网络层协议负责确定\underline{把收到的包从哪条路经转发(forwarding)出去}，即\textemph{路由选择}(routing)功能。
具体的传送则由数据链路层和物理层负责。
...
\begin{itemize}
\item \textemph{可路由的}(routable)：全局地址，按层分配（三层）
...
DHCP协议(Dynamic Host Configuration Protocol)用于主机在加入网络时\textemph{动态租用}IP地址，基于UDP，四个步骤如下：
\begin{itemize}
...
\begin{itemize}
	...
	\end{itemize}
	\item 每到一个路由器都将帧拆出来，再重新封装，\textemph{目的和源MAC地址}（上一跳路由器MAC地址）全要发生变化
\end{itemize}
...
详细过程如下
\begin{itemize}
	\item 发现邻居：每10秒向邻居发送Hello分组，如果\textemph{40s(dead interval)}都收不到邻居发来的Hello分组，则把到邻居的链路标记为失效。多路访问网络采用多播(224.0.0.5, all OSPF routers)发送Hello分组。一个Hello分组包含\underline{优先权、已知的邻居（收到过Hello）、DR和BDR}
	\item 完全相邻：在发现邻居之后， OSPF路由器将与邻居交换链路状态数据库中的LSA，请求得到更新的或者没有的LSA。在与邻居的链路状态数据库变得完全一样时，它们就处于\underline{完全相邻状态}(fully adjacency)
	\item 生成LSA：每\textemph{30分钟}或\textemph{链路变化}时，每个OSPF路由器会生成router LSA，中转网的DR会生成network LSA
